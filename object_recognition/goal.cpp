...
int main(void) {
	...
	cv::inRange(dst, cv::Scalar(160, 150, 0), cv::Scalar(180, 255, 255), hsv_filtered180);
	double red_count = 0; //赤色を認識したピクセルの数
	red_count = cv::countNonZero(hsv_filtered15) + cv::countNonZero(hsv_filtered180);//赤色部分の面積を計算
	cv::Moments mu = cv::moments(dst_img15, false);//重心の計算結果をmuに代入
	double mc = mu.m10 / mu.m00;//重心のx座標
	double X = (mc - dst.cols / 2) * 200 / dst.cols;//正規化
